...
void	draw_line(t_lem *lem)
{
    ...
    {
        put_pixel_img(lem, lem->x_1, lem->y_1, 0x00FFFFFF);
...
void 	circleSimple(t_lem *lem, int xCenter, int yCenter)
{
    int x;
    int y;
...
void    ft_draw_connections(t_lem *lem)
{
    ...
    while (i < lem->size)
    {
        ...
        while (j < lem->size)
        {
            ...
            {
                lem->x_1 = ft_get_room(lem->head, i)->x;
                lem->y_1 = ft_get_room(lem->head, i)->y;
                lem->x_2 = ft_get_room(lem->head, j)->x;
                lem->y_2 = ft_get_room(lem->head, j)->y;
                draw_line(lem);
            }
            j++;
        }
...
void    ft_print_lems_in_rooms(t_lem *lem, t_room *room)
{
    ...
    if (room->active_lem != 0)
    {
        ...
        mlx_string_put(lem->mlx, lem->win, (int)room->x - 10,
                       (int)(room->y) - 5, 0x00FF0000, "L");
        mlx_string_put(lem->mlx, lem->win, (int)(room->x),
                       (int)(room->y - 5), 0x00FF0000, number);
        free(number);
    }
}
...
int 	ft_visual(t_lem *lem)
{
    ...
    while (start)
    {
        mlx_string_put(lem->mlx, lem->win, (int)start->x,
                       (int)(start->y - lem->radius * 2 - 10), 0x00FF00FF, start->name);
        ft_print_lems_in_rooms(lem, start);
        start = start->next;
    }
    lem->flag = 1;
    number = ft_itoa((int)lem->ants);
    mlx_string_put(lem->mlx, lem->win, 10, 10, 0x00FFFFF, number);
    free(number);
    return (0);
}